  ...
  return (
    <ul className="flex flex-wrap gap-1.5">
      {shown.map((item) => (
        <li
          key={item}
          className="rounded-md border border-line bg-fill-card px-2 py-1 font-mono text-[11px] text-fg-2"
        >
          {item}
        </li>
      ))}
...
export function DemoChip({ number, className = "" }: { number: string; className?: string }) {
  ...
  return (
    <a
      ...
      className={`inline-flex w-fit items-center gap-2 rounded-lg border border-[rgba(37,211,102,0.3)] bg-[rgba(37,211,102,0.08)] px-3 py-1.5 text-[13px] font-semibold text-[var(--wa)] transition-colors duration-300 hover:bg-[rgba(37,211,102,0.14)] ${className}`}
    >
